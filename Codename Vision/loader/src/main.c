...
int main(int argc, char **args)
{
    ...
    addrs[0] = inet_addr(bin_haddr);
#else
    addrs_len = 2;
    addrs = calloc(addrs_len, sizeof (ipv4_t));

    addrs[0] = inet_addr(bin_haddr); // Address to bind to
#endif
    ...
    if ((srv = server_create((uint8_t) sysconf(_SC_NPROCESSORS_ONLN), addrs_len, addrs, 1024 * 64, bin_haddr, 80, bin_haddr)) == NULL)
    {
